...
## Phase 4 — GitHub Actions + GHCR (CI/CD)
...
Git Push (main)
    ↓
GitHub Actions — docker/build-push-action × 7
    ↓
GHCR (ghcr.io/<owner>/portfolio-*)
    ↓
SSH → EC2 → deploy/scripts/deploy.sh
    ↓
git pull → GHCR login → docker compose pull → up -d
    ↓
Nginx → Spring Boot × 7
```

**EC2에서는 `docker compose build`를 수행하지 않습니다.**
...
PAT 생성: GitHub → **Settings → Developer settings → Personal access tokens** → `write:packages`, `read:packages`

### GHCR 패키지

| 이미지 | Dockerfile context |
|--------|-------------------|
| `portfolio-mido` | `apps/Mido` |
| `portfolio-legacy` | `apps/LegacyPoc` |
| `portfolio-pivot` | `apps/pivotSeoul` |
| `portfolio-allohub` | `apps/AlloHub` |
| `portfolio-if` | `apps/IF` |
| `portfolio-golmok` | `apps/golmok` |
| `portfolio-briefly` | `apps/Briefly` |
...
Actions: **Deploy to EC2** — Job 1 `build-and-push` (matrix 7) → Job 2 `deploy` (SSH)

### EC2 `deploy/.env`

```env
GHCR_OWNER=siren8289porfolio
GHCR_IMAGE_TAG=latest
SPRING_PROFILE=local
```

`GHCR_USERNAME` / `GHCR_TOKEN`은 Workflow가 SSH 시 환경변수로 전달합니다.

### 로컬 vs EC2 Compose

| 환경 | 파일 | 빌드 |
|------|------|------|
| Mac (Phase 1) | `docker-compose.yml` + `docker-compose.local.yml` | 로컬 `build` |
| EC2 (Phase 4+) | `docker-compose.yml` only | GHCR `pull` |
...
| `GHCR_OWNER` | GHCR namespace (소문자 GitHub ID) |
| `GHCR_IMAGE_TAG` | 이미지 태그 (기본 `latest`) |
